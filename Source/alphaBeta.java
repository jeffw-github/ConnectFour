...
public class alphaBeta extends AIModule
{
		...
		while(!terminate)
		{
			...
			for(int move: moves)
			{
				if(move > -1)
				{
					...
					int newMax = Math.max( max, minValue(state, 1, evaluation(state), alpha, beta));
	...
	// minVal
	private int minValue(final GameStateModule state, int curLayer, int initial, int alpha, int beta)
	{
		// if terminal state
		if(state.isGameOver())
		{
		...
		for(int move : moves)
		{
			...
			{
				state.makeMove(move);
				min =  Math.min(min, maxValue(state, curLayer + 1, (int)(initial + evaluation(state)), alpha, beta));
				state.unMakeMove();
				if(alpha >= min)
					return min;
	...
	// maxVal
	private int maxValue(final GameStateModule state, int curLayer, int initial, int alpha, int beta)
	{
		// if terminal state
		if(state.isGameOver())
		{
		...
		for(int move:moves)
		{
			...
			{
				state.makeMove(move);
				max =  Math.max(max, minValue(state, curLayer + 1, (int)(initial + evaluation(state)), alpha, beta));
				state.unMakeMove();
				if(max >= beta)
					return max;
